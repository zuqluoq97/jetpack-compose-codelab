...
import com.example.ltdung.jetpackcompose.ui.state.copyWithResult
 ...
 * along with an event to refresh (or call [block] again), and another even to clear error results.
 *
 * It is mentioned that you destructure the return at the call site. Here is an example usage that
 * calls dataSource.loadData() and then displays a UI based on the result.
 *
 * ```
 * val (result, onRefresh, onClearError) = produceUiState(dataSource) { loadData() }
 ...
 * @param block suspend lambda that produces a single value from the data source
    ...
    val result = produceState(UiState<T>(loading = true), producer, key) {
        ...
        launch {
            ...
            for (clearEvent in errorClearChannel) {
                // suspend when errorClearChannel is empty, and resume when the
                // next value is offered or sent to the channel.
                value = value.copy(exception = null)
            }
        }

        // this for-loop wil loop until the [produceState] coroutine is cancelled.
